<template>
  <section class="page">
    ...
    <!-- 新建/编辑弹窗 -->
    <el-dialog v-model="dialogVisible" :title="editId ? '编辑学籍' : '新建学籍'" width="480px">
...
const form = ref({ studentNo: '', name: '', gender: '男', gradeName: '', classId: null, className: '', status: 'ACTIVE' });
...
const openCreate = () => {
  editId.value = null;
  form.value = { studentNo: '', name: '', gender: '男', gradeName: '', classId: null, className: '', status: 'ACTIVE' };
  dialogVisible.value = true;
};
const openEdit = (row) => {
  editId.value = row.id;
  form.value = { studentNo: row.studentNo, name: row.name, gender: row.gender, gradeName: row.gradeName,
    classId: row.classId, className: row.className, status: row.status };
  dialogVisible.value = true;
};
...
const handleSave = async () => {
  ...
  if (editId.value) await http.put(`/stu/students/${editId.value}`, form.value);
  else await http.post('/stu/students', form.value);
